...
namespace UnityEditor.PostProcessing
{
    ...
    sealed class TrackballGroupDrawer : PropertyDrawer
    {
        ...
        static Dictionary<string, MethodInfo> m_TrackballMethods =
        ...
        void OnWheelGUI(Rect position, int size, SerializedProperty property)
        {
            ...
            if (Event.current.type == EventType.Repaint)
            {
                ...
                FxStyles.wheelThumb.Draw(
                    new Rect(
                        wheelDrawArea.x + hsize + thumbPos.x - thumbSizeH.x,
                        wheelDrawArea.y + hsize + thumbPos.y - thumbSizeH.y,
                        thumbSize.x,
                        thumbSize.y
                    ),
                    false,
        ...
        bool TryGetDisplayValue(Color color, SerializedProperty property, out Vector3 output)
        {
            ...
            if (!m_TrackballMethods.TryGetValue(property.name, out method))
            {
                ...
                var attr = (TrackballAttribute)
                    field.GetCustomAttributes(typeof(TrackballAttribute), false)[0];
            ...
            output = (Vector3)
                method.Invoke(property.serializedObject.targetObject, new object[] { color });
            return true;
        }
