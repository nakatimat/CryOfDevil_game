    ...
    public sealed class CurveEditor
    {
        ...
        void OnCurveGUI(Rect rect, SerializedProperty curve, CurveState state)
        {
            ...
            for (int k = 0; k < length; k++)
            {
                ...
                if (state.editable)
                {
                    ...
                    {
                        if (
                            inTangentHitRect.Contains(e.mousePosition)
                            && (k > 0 || state.loopInBounds)
                        )
                        {
                            SelectKeyframe(curve, k);
                            m_EditMode = EditMode.TangentEdit;
                            ...
                            outTangentHitrect.Contains(e.mousePosition)
                            && (k < length - 1 || state.loopInBounds)
                        )
                        {
                            SelectKeyframe(curve, k);
                            m_EditMode = EditMode.TangentEdit;
        ...
        void OnGeneralUI(Rect rect)
        {
            ...
            if (e.type == EventType.MouseDown)
            {
                ...
                else if (!used && e.button == 1)
                {
                    ...
                    var menu = new GenericMenu();
                    menu.AddItem(
                        new GUIContent("Add Key At Position"),
                        false,
                        () => ContextMenuAddKey(hit, false)
                    );
                    menu.AddItem(
                        new GUIContent("Add Key On Curves"),
                        false,
                        () => ContextMenuAddKey(hit, true)
                    );
                    menu.ShowAsContext();
                }
        ...
        Vector3 CurveTangentToCanvas(float tangent)
        {
            if (!float.IsInfinity(tangent))
            {
                var bounds = settings.bounds;
                float ratio =
                    (m_CurveArea.width / m_CurveArea.height)
        ...
        Vector3[] BezierSegment(Keyframe start, Keyframe end)
        {
            ...
            segment[1] = CurveToCanvas(
                new Vector3(
                    middle,
                    ProjectTangent(start.time, start.value, start.outTangent, middle)
                )
            );
