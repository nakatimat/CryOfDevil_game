    ...
    public static class EditorGUIHelper
    {
        ...
        public static bool Header(string title, SerializedProperty group, Action resetAction)
        {
            ...
            var popupRect = new Rect(
                rect.x + rect.width - FxStyles.paneOptionsIcon.width - 5f,
        ...
        {
            var field = ReflectionUtils.GetFieldInfoFromPath(
                enabledField.serializedObject.targetObject,
                enabledField.propertyPath
            );
            object parent = null;
            ...
            {
                var attr = (GetSetAttribute)
                    field.GetCustomAttributes(typeof(GetSetAttribute), false)[0];
                parent = ReflectionUtils.GetParentObject(
                    enabledField.propertyPath,
                    enabledField.serializedObject.targetObject
                );
        ...
        static void CopySettings(SerializedProperty settings)
        {
            ...
            var settingsStruct = ReflectionUtils.GetFieldValueFromPath(
                settings.serializedObject.targetObject,
                ref t,
        ...
        static void PasteSettings(SerializedProperty settings)
        {
            ...
            var json = EditorGUIUtility.systemCopyBuffer.Substring(
                field.FieldType.ToString().Length + 1
            );
            var obj = JsonUtility.FromJson(json, field.FieldType);
            var parent = ReflectionUtils.GetParentObject(
                settings.propertyPath,
                settings.serializedObject.targetObject
            );
            field.SetValue(
