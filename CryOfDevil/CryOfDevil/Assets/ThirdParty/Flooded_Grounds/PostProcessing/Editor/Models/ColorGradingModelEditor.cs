    ...
    public class ColorGradingModelEditor : PostProcessingModelEditor
    {
        ...
        public override void OnEnable()
        {
            ...
            m_ChannelMixer = new ChannelMixerSettings
            {
                ...
                currentEditingChannel = FindSetting(
                    (Settings x) => x.channelMixer.currentEditingChannel
                ),
            };
        ...
        {
            int tid = EditorGUILayout.Popup(
                EditorGUIHelper.GetContent("Tonemapper"),
                m_Tonemapping.tonemapper.intValue,
                s_Tonemappers
            );
            ...
            if (tid == (int)Tonemapper.Neutral)
            {
                ...
                EditorGUILayout.PropertyField(
                    m_Tonemapping.neutralBlackIn,
                    EditorGUIHelper.GetContent("Black In")
                );
                EditorGUILayout.PropertyField(
                    m_Tonemapping.neutralWhiteIn,
                    EditorGUIHelper.GetContent("White In")
                );
                EditorGUILayout.PropertyField(
                    m_Tonemapping.neutralBlackOut,
                    EditorGUIHelper.GetContent("Black Out")
                );
                EditorGUILayout.PropertyField(
                    m_Tonemapping.neutralWhiteOut,
                    EditorGUIHelper.GetContent("White Out")
                );
                EditorGUILayout.PropertyField(
                    m_Tonemapping.neutralWhiteLevel,
                    EditorGUIHelper.GetContent("White Level")
                );
                EditorGUILayout.PropertyField(
                    m_Tonemapping.neutralWhiteClip,
                    EditorGUIHelper.GetContent("White Clip")
                );
            }
        ...
        void DoBasicGUI()
        {
            EditorGUILayout.PropertyField(
                m_Basic.exposure,
                EditorGUIHelper.GetContent("Post Exposure (EV)")
            );
        ...
        void DoChannelMixerGUI()
        {
            ...
            EditorGUI.BeginChangeCheck();
            {
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    if (
                        GUILayout.Toggle(
                            currentChannel == 0,
                            EditorGUIHelper.GetContent("Red|Red output channel."),
                    ...
                    if (
                        GUILayout.Toggle(
                            currentChannel == 1,
                            EditorGUIHelper.GetContent("Green|Green output channel."),
                    ...
                    if (
                        GUILayout.Toggle(
                            currentChannel == 2,
                            EditorGUIHelper.GetContent("Blue|Blue output channel."),
            ...
            v.x = EditorGUILayout.Slider(
                EditorGUIHelper.GetContent(
                    "Red|Modify influence of the red channel within the overall mix."
                ),
                v.x,
                -2f,
                2f
            ...
            v.y = EditorGUILayout.Slider(
                EditorGUIHelper.GetContent(
                    "Green|Modify influence of the green channel within the overall mix."
                ),
                v.y,
                -2f,
                2f
            ...
            v.z = EditorGUILayout.Slider(
                EditorGUIHelper.GetContent(
                    "Blue|Modify influence of the blue channel within the overall mix."
                ),
                v.z,
                -2f,
                2f
        ...
        bool SpecialToggle(bool value, string name, out bool rightClicked)
        {
            var rect = GUILayoutUtility.GetRect(
                EditorGUIHelper.GetContent(name),
                EditorStyles.toolbarButton
            );

            var e = Event.current;
            rightClicked = (
                e.type == EventType.MouseUp && rect.Contains(e.mousePosition) && e.button == 1
            );
        ...
        void DoCurvesGUI()
        {
            ...
            )
            {
                ...
                {
                    curveEditingId = EditorGUILayout.Popup(
                        m_Curves.currentEditingCurve.intValue,
                        s_Curves,
                    ...
                    if (curveEditingId == 0)
                    {
                        ...
                        if (y)
                            SetCurveVisible(m_Curves.master);
                        if (r)
                            SetCurveVisible(m_Curves.red);
                        if (g)
                            SetCurveVisible(m_Curves.green);
                        if (b)
                            SetCurveVisible(m_Curves.blue);
                    ...
                    if (GUILayout.Button("Reset", EditorStyles.toolbarButton))
                    {
                        switch (curveEditingId)
                        {
                            case 0:
                                if (y)
                                    m_Curves.master.animationCurveValue = AnimationCurve.Linear(
                                        0f,
                                        0f,
                                        1f,
                                        1f
                                ...
                                if (r)
                                    m_Curves.red.animationCurveValue = AnimationCurve.Linear(
                                        0f,
                                        0f,
                                        1f,
                                        1f
                                ...
                                if (g)
                                    m_Curves.green.animationCurveValue = AnimationCurve.Linear(
                                        0f,
                                        0f,
                                        1f,
                                        1f
                                ...
                                if (b)
                                    m_Curves.blue.animationCurveValue = AnimationCurve.Linear(
                                        0f,
                                        0f,
                                        1f,
                                        1f
                ...
                if (Event.current.type == EventType.Repaint)
                {
                    ...
                    if (s_MaterialSpline == null)
                        s_MaterialSpline = new Material(
                            Shader.Find("Hidden/Post FX/UI/Curve Background")
                        )
                        {
