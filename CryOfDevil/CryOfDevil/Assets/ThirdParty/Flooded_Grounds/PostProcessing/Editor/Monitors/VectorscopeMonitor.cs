    ...
    public class VectorscopeMonitor : PostProcessingMonitor
    {
        ...
        public VectorscopeMonitor()
        {
            m_ComputeShader = EditorResources.Load<ComputeShader>(
                "Monitors/VectorscopeCompute.compute"
            );
        }
        ...
        public override void OnMonitorSettings()
        {
            ...
            refreshOnPlay = GUILayout.Toggle(
                ...
                new GUIContent(
                    FxStyles.playIcon,
                    "Keep refreshing the vectorscope in play mode; this may impact performances."
                ),
                FxStyles.preButton
            );
            ...
            showBackground = GUILayout.Toggle(
                showBackground,
                new GUIContent(FxStyles.checkerIcon, "Show an YUV background in the vectorscope."),
                FxStyles.preButton
            );
        ...
        public override void OnMonitorGUI(Rect r)
        {
            if (Event.current.type == EventType.Repaint)
            {
                ...
                if (m_VectorscopeTexture != null)
                {
                    ...
                    Handles.color = color;
                    Handles.DrawLine(
                        new Vector2(midX, m_MonitorAreaRect.y),
                        new Vector2(midX, m_MonitorAreaRect.y + m_MonitorAreaRect.height)
                    );
                    Handles.DrawLine(
                        new Vector2(m_MonitorAreaRect.x, midY),
        ...
        void ComputeVectorscope(RenderTexture source)
        {
            ...
            cs.Dispatch(
                kernel,
                Mathf.CeilToInt(source.width / 32f),
                Mathf.CeilToInt(source.height / 32f),
                1
            );
            ...
            if (
                m_VectorscopeTexture == null
                || m_VectorscopeTexture.width != source.width
                || m_VectorscopeTexture.height != source.height
            )
            {
                GraphicsUtils.Destroy(m_VectorscopeTexture);
                m_VectorscopeTexture = new RenderTexture(
                    source.width,
                    source.height,
                    0,
            ...
            m_Material.SetVector(
                "_Size",
                new Vector2(m_VectorscopeTexture.width, m_VectorscopeTexture.height)
            );
        }
    }
}
