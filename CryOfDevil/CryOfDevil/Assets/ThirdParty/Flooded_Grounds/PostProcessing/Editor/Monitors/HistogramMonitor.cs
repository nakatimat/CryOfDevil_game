    ...
    public class HistogramMonitor : PostProcessingMonitor
    {
        ...
        public HistogramMonitor()
        {
            m_ComputeShader = EditorResources.Load<ComputeShader>(
                "Monitors/HistogramCompute.compute"
            );
        }
        ...
        public override void OnMonitorSettings()
        {
            ...
            mode = (HistogramMode)
                EditorGUILayout.EnumPopup(mode, FxStyles.preDropdown, GUILayout.MaxWidth(100f));

            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(
                    m_BaseEditor.serializedObject.targetObject,
                    "Histogram Settings Changed"
                );
            ...
            if (Event.current.type == EventType.Repaint)
            {
                ...
                if (m_HistogramTexture != null)
                {
                    ...
                    if (m_MonitorSettings.histogramMode == HistogramMode.RGBSplit)
                    {
                        ...
                        var E = new Vector3(
                            A.x + m_MonitorAreaRect.width + 2f,
                            m_MonitorAreaRect.y - 1f
                        );
                        ...
                        GUI.color = color;
                        GUI.Label(
                            new Rect(L.x - 15f, L.y + kTickSize - 4f, 30f, 30f),
                            "0.0",
                            FxStyles.tickStyleCenter
                        );
                        GUI.Label(
                            new Rect(J.x - 15f, J.y + kTickSize - 4f, 30f, 30f),
                            "0.5",
                            FxStyles.tickStyleCenter
                        );
                        GUI.Label(
                            new Rect(H.x - 15f, H.y + kTickSize - 4f, 30f, 30f),
                            "1.0",
                            FxStyles.tickStyleCenter
                        );
                    }
                    else
                    {
                        ...
                        var E = new Vector3(
                            A.x + m_MonitorAreaRect.width + 1f,
                            m_MonitorAreaRect.y
                        );
        ...
        void ComputeHistogram(RenderTexture source)
        {
            ...
            cs.Dispatch(
                kernel,
                Mathf.CeilToInt(source.width / 16f),
                Mathf.CeilToInt(source.height / 16f),
                1
            );
            ...
            {
                GraphicsUtils.Destroy(m_HistogramTexture);
                m_HistogramTexture = new RenderTexture(
                    source.width,
                    source.height,
                    0,
            ...
            m_Material.SetVector(
                "_Size",
                new Vector2(m_HistogramTexture.width, m_HistogramTexture.height)
            );
