...
namespace UnityEditor.PostProcessing
{
    public class WaveformMonitor : PostProcessingMonitor
    {
        ...
        {
            m_ComputeShader = EditorResources.Load<ComputeShader>(
                "Monitors/WaveformCompute.compute"
            );
        }
        ...
        public override void OnMonitorSettings()
        {
            ...
            refreshOnPlay = GUILayout.Toggle(
                refreshOnPlay,
                new GUIContent(
                    FxStyles.playIcon,
                    "Keep refreshing the waveform in play mode; this may impact performances."
                ),
                FxStyles.preButton
            );

            exposure = GUILayout.HorizontalSlider(
                exposure,
                0.05f,
                0.3f,
                FxStyles.preSlider,
                FxStyles.preSliderThumb,
                GUILayout.Width(40f)
            );

            Y = GUILayout.Toggle(
                Y,
                new GUIContent("Y", "Show the luminance waveform only."),
                FxStyles.preButton
            );
            ...
            R = GUILayout.Toggle(
                R,
                new GUIContent("R", "Show the red waveform."),
                FxStyles.preButton
            );
            G = GUILayout.Toggle(
                G,
                new GUIContent("G", "Show the green waveform."),
                FxStyles.preButton
            );
            B = GUILayout.Toggle(
                B,
                new GUIContent("B", "Show the blue waveform."),
                FxStyles.preButton
            );
            ...
            {
                Undo.RecordObject(
                    m_BaseEditor.serializedObject.targetObject,
                    "Waveforme Settings Changed"
                );
        ...
        public override void OnMonitorGUI(Rect r)
        {
            if (Event.current.type == EventType.Repaint)
            {
                ...
                if (
                    Mathf.Approximately(m_MonitorAreaRect.width, 0)
                    && Mathf.Approximately(m_MonitorAreaRect.height, 0)
                )
                    InternalEditorUtility.RepaintAllViews();
                ...
                if (m_WaveformTexture != null)
                {
                    ...
                    GUI.color = color;
                    GUI.Label(
                        new Rect(A.x - kTickSize - 34f, A.y - 15f, 30f, 30f),
                        "1.0",
                        FxStyles.tickStyleRight
                    );
                    GUI.Label(
                        new Rect(O.x - kTickSize - 34f, O.y - 15f, 30f, 30f),
                        "0.5",
                        FxStyles.tickStyleRight
                    );
                    GUI.Label(
                        new Rect(M.x - kTickSize - 34f, M.y - 15f, 30f, 30f),
                        "0.0",
                        FxStyles.tickStyleRight
                    );

                    GUI.Label(
                        new Rect(E.x + kTickSize + 4f, E.y - 15f, 30f, 30f),
                        "1.0",
                        FxStyles.tickStyleLeft
                    );
                    GUI.Label(
                        new Rect(G.x + kTickSize + 4f, G.y - 15f, 30f, 30f),
                        "0.5",
                        FxStyles.tickStyleLeft
                    );
                    GUI.Label(
                        new Rect(I.x + kTickSize + 4f, I.y - 15f, 30f, 30f),
                        "0.0",
                        FxStyles.tickStyleLeft
                    );

                    GUI.Label(
                        new Rect(M.x - 15f, M.y + kTickSize - 4f, 30f, 30f),
                        "0.0",
                        FxStyles.tickStyleCenter
                    );
                    GUI.Label(
                        new Rect(K.x - 15f, K.y + kTickSize - 4f, 30f, 30f),
                        "0.5",
                        FxStyles.tickStyleCenter
                    );
                    GUI.Label(
                        new Rect(I.x - 15f, I.y + kTickSize - 4f, 30f, 30f),
                        "1.0",
                        FxStyles.tickStyleCenter
                    );
                }
        ...
        void ComputeWaveform(RenderTexture source)
        {
            ...
            var channels = m_MonitorSettings.waveformY
                ...
                : new Vector4(
                    m_MonitorSettings.waveformR ? 1f : 0f,
                    m_MonitorSettings.waveformG ? 1f : 0f,
                    m_MonitorSettings.waveformB ? 1f : 0f,
                    0f
                );
            ...
            if (
                m_WaveformTexture == null
                || m_WaveformTexture.width != source.width
                || m_WaveformTexture.height != source.height
            )
            {
                GraphicsUtils.Destroy(m_WaveformTexture);
                m_WaveformTexture = new RenderTexture(
                    source.width,
                    source.height,
                    0,
            ...
            m_Material.SetVector(
                "_Size",
                new Vector2(m_WaveformTexture.width, m_WaveformTexture.height)
            );
