    ...
    public class ParadeMonitor : PostProcessingMonitor
    {
        ...
        public override void OnMonitorSettings()
        {
            ...
            refreshOnPlay = GUILayout.Toggle(
                ...
                new GUIContent(
                    FxStyles.playIcon,
                    "Keep refreshing the parade in play mode; this may impact performances."
                ),
                FxStyles.preButton
            );
        ...
        public override void OnMonitorGUI(Rect r)
        {
            if (Event.current.type == EventType.Repaint)
            {
                ...
                if (m_WaveformTexture != null)
                {
                    ...
                    GUI.Label(
                        new Rect(K.x - kTickSize - 34f, K.y - 15f, 30f, 30f),
                        "0.0",
                        FxStyles.tickStyleRight
                    );

                    GUI.Label(
                        new Rect(D.x + kTickSize + 4f, D.y - 15f, 30f, 30f),
                        "1.0",
                        FxStyles.tickStyleLeft
                    );
                    GUI.Label(
                        new Rect(F.x + kTickSize + 4f, F.y - 15f, 30f, 30f),
                        "0.5",
                        FxStyles.tickStyleLeft
                    );
                    GUI.Label(
                        new Rect(H.x + kTickSize + 4f, H.y - 15f, 30f, 30f),
                        "0.0",
                        FxStyles.tickStyleLeft
                    );
                }
