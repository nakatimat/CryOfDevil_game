    ...
    public sealed class GrainComponent : PostProcessingComponentRenderTexture<GrainModel>
    {
        ...
        public override void Prepare(Material uberMaterial)
        {
            ...
            uberMaterial.SetVector(
                Uniforms._Grain_Params1,
                new Vector2(settings.luminanceContribution, settings.intensity * 20f)
            );
            uberMaterial.SetVector(
                Uniforms._Grain_Params2,
                new Vector4(
                    (float)context.width / (float)m_GrainLookupRT.width / settings.size,
                    (float)context.height / (float)m_GrainLookupRT.height / settings.size,
                    rndOffsetX,
                    rndOffsetY
                )
            );
