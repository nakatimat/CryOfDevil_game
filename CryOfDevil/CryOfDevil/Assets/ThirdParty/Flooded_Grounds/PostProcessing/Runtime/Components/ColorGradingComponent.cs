namespace UnityEngine.PostProcessing
{
    ...
    public sealed class ColorGradingComponent
        : PostProcessingComponentRenderTexture<ColorGradingModel>
    {
        static class Uniforms
        {
            internal static readonly int _LutParams = Shader.PropertyToID("_LutParams");
            internal static readonly int _NeutralTonemapperParams1 = Shader.PropertyToID(
                "_NeutralTonemapperParams1"
            );
            internal static readonly int _NeutralTonemapperParams2 = Shader.PropertyToID(
            ...
            internal static readonly int _ChannelMixerGreen = Shader.PropertyToID(
                "_ChannelMixerGreen"
            );
            internal static readonly int _ChannelMixerBlue = Shader.PropertyToID(
                "_ChannelMixerBlue"
            );
        ...
        {
            get { return model.enabled && !context.interrupted; }
        }
        ...
        void GenerateLut()
        {
            ...
            if (!IsLogLutValid(model.bakedLut))
            {
                ...
                model.bakedLut = new RenderTexture(
                    k_InternalLogLutSize * k_InternalLogLutSize,
            ...
            switch (tonemapping.tonemapper)
            {
                case ColorGradingModel.Tonemapper.Neutral:
                {
                    ...
                    lutMaterial.SetVector(
                        Uniforms._NeutralTonemapperParams1,
                        new Vector4(a, b, c, d)
                    );
                    lutMaterial.SetVector(
                        Uniforms._NeutralTonemapperParams2,
                        new Vector4(
                            e,
                            f,
                            tonemapping.neutralWhiteLevel,
        ...
        public override void Prepare(Material uberMaterial)
        {
            ...
            uberMaterial.SetVector(
                Uniforms._LogLut_Params,
                new Vector3(1f / bakedLut.width, 1f / bakedLut.height, bakedLut.height - 1f)
            );

            float ev = Mathf.Exp(
                model.settings.basic.postExposure * 0.69314718055994530941723212145818f
            );
        ...
        public void OnGUI()
        {
            var bakedLut = model.bakedLut;
            var rect = new Rect(
                context.viewport.x * Screen.width + 8f,
                8f,
                bakedLut.width,
                bakedLut.height
            );
