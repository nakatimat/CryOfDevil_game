...
namespace UnityEngine.PostProcessing
{
    ...
    public sealed class MotionBlurComponent : PostProcessingComponentCommandBuffer<MotionBlurModel>
    {
        static class Uniforms
        {
            ...
            internal static readonly int _RcpMaxBlurRadius = Shader.PropertyToID(
                "_RcpMaxBlurRadius"
            );
            ...
            internal static readonly int _History1ChromaTex = Shader.PropertyToID(
                "_History1ChromaTex"
            );
            internal static readonly int _History2ChromaTex = Shader.PropertyToID(
                "_History2ChromaTex"
            );
            internal static readonly int _History3ChromaTex = Shader.PropertyToID(
                "_History3ChromaTex"
            );
            internal static readonly int _History4ChromaTex = Shader.PropertyToID(
                "_History4ChromaTex"
            );
        ...
        public class ReconstructionFilter
        {
            ...
            )
            {
                ...
                cb.GetTemporaryRT(
                    tile,
                    context.width / tileSize,
                    context.height / tileSize,
                    0,
                    FilterMode.Point,
                    m_VectorRTFormat,
                    RenderTextureReadWrite.Linear
                ...
                cb.SetGlobalFloat(
                    Uniforms._LoopCount,
                    Mathf.Clamp(settings.sampleCount / 2, 1, 64)
                );
        ...
        public override void PopulateCommandBuffer(CommandBuffer cb)
        {
            ...
            if (settings.shutterAngle > 0f && settings.frameBlending > 0f)
            {
                ...
                reconstructionFilter.ProcessImage(
                    context,
                    cb,
                    ref settings,
                ...
                frameBlendingFilter.BlendFrames(
                    cb,
                    settings.frameBlending,
                    tempRT,
                    BuiltinRenderTextureType.CameraTarget,
                    material
                );
