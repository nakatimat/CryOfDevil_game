    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        if (targetMat.HasProperty("_BumpSpecMap"))
        {
            if (targetMat.GetTexture("_BumpSpecMap") == null)
            {
                targetMat.SetTexture(
                    "_BumpSpecMap",
                    Resources.Load("CTI_default_normal_spec") as Texture2D
                );
            }
        }
        ...
        if (targetMat.HasProperty("_AverageCol"))
        {
            ...
            if (GUILayout.Button("Get average Color"))
            {
                if (targetMat.HasProperty("_MainTex") && targetMat.GetTexture("_MainTex") != null)
                {
                    ...
                    targetMat.HasProperty("_MainTexArray")
                    && targetMat.GetTexture("_MainTexArray") != null
                )
                {
                    Texture2DArray sourceTex =
                        targetMat.GetTexture("_MainTexArray") as Texture2DArray;
